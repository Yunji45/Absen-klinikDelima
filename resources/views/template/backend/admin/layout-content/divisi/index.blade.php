@extends('template.layout.app.main') 
@section('tabel')
<section class="section">
          <div class="section-header">
            <div class="section-header-back">
              <a href="features-settings.html" class="btn btn-icon"><i class="fas fa-arrow-left"></i></a>
            </div>
            <h1>Divisi Settings</h1>
            <div class="section-header-breadcrumb">
              <div class="breadcrumb-item active"><a href="#">Dashboard</a></div>
              <div class="breadcrumb-item active"><a href="#">Settings</a></div>
              <div class="breadcrumb-item">Divisi Settings</div>
            </div>
          </div>

          <div class="section-body">
            <h2 class="section-title">All About Divisi Settings</h2>
            <p class="section-lead">
              You can adjust all general settings here
            </p>

            <div id="output-status"></div>
            <div class="row">
              <div class="col-md-4">
                <div class="card">
                  <div class="card-header">
                    <h4>Jump To</h4>
                  </div>
                  <div class="card-body">
                    <ul class="nav nav-pills flex-column">
                      <li class="nav-item"><a href="{{route('setting-content.beranda')}}" class="nav-link">Divisi</a></li>
                      <li class="nav-item"><a href="{{route('setting-content.tentang')}}" class="nav-link">Tentang</a></li>
                      <li class="nav-item"><a href="{{route('setting-content.layanan')}}" class="nav-link">Divisi</a></li>
                      <li class="nav-item"><a href="{{route('setting-content.divisi')}}" class="nav-link active">Divisi</a></li>
                      <li class="nav-item"><a href="{{route('setting-content.dokter')}}" class="nav-link">Dokter</a></li>
                      <li class="nav-item"><a href="{{route('setting-content.faq')}}" class="nav-link">FAQ & Kontak</a></li>
                    </ul>
                  </div>
                </div>
              </div>
              <div class="col-md-8">
                <form action="{{route('setting-content.divisi.save')}}" id="setting-form" method="POST" enctype="multipart/form-data">
                  @csrf
                  <div class="card" id="settings-card">
                    <div class="card-header">
                      <h4>Divisi Settings</h4>
                    </div>
                    <div class="card-body">
                      <p class="text-muted">Divisi settings such as, site title, site description, address and so on.</p>
                      <div class="form-group row align-items-center">
                        <label for="site-title" class="form-control-label col-sm-3 text-md-right">Nama Divisi</label>
                        <div class="col-sm-6 col-md-9">
                          <input type="text" name="nama_divisi" class="form-control" id="nama_divisi">
                        </div>
                      </div>
                      <div class="form-group row align-items-center">
                        <label for="site-description" class="form-control-label col-sm-3 text-md-right">Deskripsi Singkat</label>
                        <div class="col-sm-6 col-md-9">
                          <textarea class="form-control" name="deskripsi_singkat" id="deskripsi_singkat"></textarea>
                        </div>
                      </div>
                      <div class="form-group row align-items-center">
                        <label for="site-description" class="form-control-label col-sm-3 text-md-right">Deskripsi Divisi</label>
                        <div class="col-sm-6 col-md-9">
                          <textarea class="form-control" name="deskripsi_divisi" id="deskripsi_divisi"></textarea>
                        </div>
                      </div>
                      <div class="form-group row align-items-center">
                        <label for="site-title" class="form-control-label col-sm-3 text-md-right">Nama Divisi</label>
                        <div class="col-sm-6 col-md-9">
                          <input type="file" name="foto_divisi" class="form-control" id="foto_divisi">
                        </div>
                      </div>
                    </div>
                    <div class="card-footer bg-whitesmoke text-md-right">
                      <button class="btn btn-primary" id="save-btn">Save Changes</button>
                      <button class="btn btn-secondary" type="button">Reset</button>
                    </div>
                  </div>
                </form>
              </div>
            </div>
            <div class="row row-cols-1 row-cols-md-3 g-4">
                @foreach($divisi as $item)
                <div class="col">
                    <div class="card h-100">
                    <img src="{{ asset(Storage::url('content-divisi/'.$item->foto_divisi)) }}" class="card-img-top" alt="...">
                    <div class="card-body">
                        <h5 class="card-title">{{$item->nama_divisi}}</h5>
                        <p class="card-text">{{$item->deskripsi_divisi}}</p>
                        <a href="{{route('setting-content.divisi.edit',$item->id)}}" class="btn btn-success">Edit</a>
                        <a href="{{route('setting-content.divisi.delete',$item->id)}}" class="btn btn-danger">Hapus</a>
                    </div>
                    </div>
                </div>
                @endforeach
            </div>
  		    </div>
      	</section>
@endsection