document.addEventListener('DOMContentLoaded', function() {
    ...
    async function fetchAvailableYears() {
        try {
            const response = await fetch('https://klinikmitradelima.com/api/api-tahun-lab');
    ...
    async function fetchData(year = new Date().getFullYear()) {
        ...
        try {
            const response = await fetch(`https://klinikmitradelima.com/api/api-search-lab?year=${year}`);
            const data = await response.json();
            console.log('Data fetched:', data);
            updateChart(data.lab_per_month);
        } catch (error) {
